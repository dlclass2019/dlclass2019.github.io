...
Hence to summarize CoSen CNN, first we initialize CNN parameters $$ \theta $$ and cost matrix $$ \xi $$. After that we update $$ \xi $$ by $$ \delta F(\xi) $$ and learning rate $$ \gamma_{\xi} $$. Then update $$ \theta $$ by CNN gradients and learning rate $$ \gamma_{\theta} $$. Finally, we keeo alternating between both the updates of $$ \xi $$ and $$ \theta $$ until convergence. As a result, Cosen CNN outperformed basic CNN on different datasets.
...
For the experiment, first we create clusters for each classes in the training sets. Now for every query $$ q $$, we find its kNN cluster centroids $$ \psi = {m_i}^{k}_{i=1} $$ from all classes learned in the training stage. If all the $$ k $$ cluster neighbors belong to the same class, $$ q $$ is labelled by that class and exit. Else, we would label $$ q $$ as $$ y_q $$ based on the following formula

$$ y_q = argmax_c(min_{m_j \epsilon (q), y_i=c} D(f(q) - f(m_j)) - max_{m_i \epsilon \psi(q), y_i=c} D(f(q), f(m_i))) $$
...
3. Algorithm level methods
	...
	* Zhang C, Tan KC, Ren R. Training cost-sensitive deep belief networks on imbalance data problems. In: 2016 international joint conference on neural networks (IJCNN). 2016. p. 4362–7.
	* 3. Hybrid methods Huang C, Li Y, Loy CC, Tang X. Learning deep representation for imbalanced classifcation. In: 2016 IEEE conference on computer vision and pattern recognition (CVPR). 2016. p. 5375–84.
